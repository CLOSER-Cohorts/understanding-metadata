...
We can often (but not always) interpret the information without explanation, but we can misinterpret information without clear structure and defintions, for example, the name: Lee Thomas. Is Lee the first name or surname? Could it be Thomas Lee? Machines stuggle even more, a computer does not: understand context, recognise common naming patterns or interpret formatting clues, so it cannot reliably determine what part of the address is the city for example. Humans can often infer meaning. Machines cannot, they rely on structure.
