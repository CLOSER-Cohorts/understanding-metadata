...
While dataset metadata provides more information about how and when the data is collected, we still don't know what the column headers mean or what the numbers represent so we can't intepret the data. Variable metadata is a key tool that enables us to do this. 
